        ...
        // When lower Quantity is clicked
            ...
            addItem.forEach(item => {                
                if (item.FoodID === id && item.Quantity >= 1){
                    item.Quantity -= 1;
                    if (item.Quantity === 0){
                        if ( window.location != 'https://VNR-canteen.netlify.app/user-orders.html'){
...
function ClientDataFlow(addToCartBtn){
    ...
    firebase.auth().onAuthStateChanged((user) => {
        if (user) {
            ...
            checkOutBtn.addEventListener('click', ()=>{
                ...
                if (addItem.length != 0){
                    ...
                    window.setTimeout(function(){
                        window.location.replace('https://VNR-canteen.netlify.app/user-orders.html')
                    }, 2600)
                } else {
                    Swal.fire({
                        icon: 'error',
            ...
            // Shows Orders
            if (window.location.href === 'https://VNR-canteen.netlify.app/user-orders.html'){
