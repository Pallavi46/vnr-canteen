  ...
  class signUpMethods {
    ...
    static authRedirecting() {
      window.setTimeout(() => {
        window.location.replace('https://VNR-canteen.netlify.app/client-side');
      }, 500)
    }
    ...
    firebaseAuthRedirect(){
      firebase.auth().onAuthStateChanged(function(user) {
        ...
        user ?  window.location.replace('https://VNR-canteen.netlify.app/client-side') : console.log('none');
      });
    }
  }
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    // Main Sign Up Form
    if (myForm) {
      myForm.addEventListener("subVNR", (e) => {
        e.preventDefault();
    ...
    if (signInForm){
      // Main Sign In Form
      signInForm.addEventListener("subVNR", (e) => {
        e.preventDefault();
      ...
      logout.forEach(btn => {
        btn.addEventListener('click', (e) => {
          ...
          firebase.auth().signOut().then(() => {
            window.location.replace("https://VNR-canteen.netlify.app/index.html")
          });
        });
      })
    }
